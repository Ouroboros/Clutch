...
- (void)main {
    @try {
        ...
        [[ClutchPrint sharedInstance] print:@"Zipping 1 %@ %@",_application.bundleURL.lastPathComponent, _application.bundleURL];
        ...
        for (NSURL *theURL in dirEnumerator)
        {
            ...
            if (_pathComponents.count > 2) {
                if ([_pathComponents[2] isEqualToString:@"SC_Info"]||[_pathComponents[2] isEqualToString:@"Watch"]||[_pathComponents[2] isEqualToString:@"Frameworks"]||[_pathComponents[2] isEqualToString:@"PlugIns"]) {
...
#if PRINT_ZIP_LOGS
                        [[ClutchPrint sharedInstance] print:@"Added 1 %@",[_localPrefix stringByAppendingPathComponent:_localPath]];
#endif
                    }else {
#if PRINT_ZIP_LOGS
                        [[ClutchPrint sharedInstance] print:@"Skipping 1 %@",[_localPrefix stringByAppendingPathComponent:_localPath]];
#endif
                    }
                }
                    ...
                    [_archive addFileToZip:theURL.path newname:[_localPrefix stringByAppendingPathComponent:_localPath]];
#if PRINT_ZIP_LOGS
                    [[ClutchPrint sharedInstance] print:@"Added 2 %@",[_localPrefix stringByAppendingPathComponent:_localPath]];
#endif
                }
                else {
#if PRINT_ZIP_LOGS
                    [[ClutchPrint sharedInstance] print:@"Skipping 2 %@",[_localPrefix stringByAppendingPathComponent:_localPath]];
#endif
                }
            }
            else {
#if PRINT_ZIP_LOGS
                [[ClutchPrint sharedInstance] print:@"Skipping 3 %@",_localPath];
#endif
            }
