...
- (BOOL)dumpBinary
{
    ...
    fmwk.binPath = swappedBinaryPath;
    fmwk.dumpPath = binaryDumpPath;
    fmwk.pages = pages;
    fmwk.ncmds = _thinHeader.header.ncmds;
    fmwk.offset = _thinHeader.offset;
    fmwk.bID = bundle.parentBundle.bundleIdentifier;
    fmwk.hashOffset = CFSwapInt32(directory.hashOffset);
    fmwk.codesign_begin = begin;
    fmwk.cryptsize = crypt.cryptoff;
    fmwk.cryptoff = crypt.cryptsize;
    fmwk.cryptlc_offset = cryptlc_offset;
    fmwk.dumpSize = fmwk.cryptoff + fmwk.cryptsize;

    BOOL successfullyDumpedFramework = [fmwk dumpBinary];
