...
- (void)main {
    @try {
        ...
        if (_application.parentBundle) {
            [[ClutchPrint sharedInstance] printDeveloper:@"Zipping 2 %@",_application.bundleURL.lastPathComponent];
        }
...
#if PRINT_ZIP_LOGS
                        [[ClutchPrint sharedInstance] print:@"Added F %@",zipPath];
#endif
                    }
        ...
        [[ClutchPrint sharedInstance] print:@"count %d / %d %@", plists.count, self.expectedBinariesCount, _application.workingPath];
